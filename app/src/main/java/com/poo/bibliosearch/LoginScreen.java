...
public class LoginScreen extends AppCompatActivity {
    ...
    public void loginButton(View view) {
        load(USERS);
        txt_email = findViewById(R.id.txt_login_document);
        txt_password = findViewById(R.id.txt_login_password);

        String document = txt_email.getText().toString();
        String password = txt_password.getText().toString();

        if (document.isEmpty() ) {
            Toast.makeText(LoginScreen.this, "Invalid or empty document", Toast.LENGTH_LONG).show();
        } else {
            int i = 0;
            ...
            while (i < items.size()) {
                ...
                if (document.equals(aux.getDocument())) {
...
//    }
    ...
    public void createItems(String TYPE) {
        ...
        switch (TYPE) {
            case "BOOKS": {
                Book item1 = new Book(items.size() + 1, (R.drawable.ic_book_1), "Book Name 1", "admin", loremIpsum, 1996, 4,10);
                items.add(item1);
                Book item2 = new Book(items.size() + 1, (R.drawable.ic_book_2), "Book Name 2", "author 2", loremIpsum, 1996, 3,2);
                items.add(item2);
            ...
            case "USERS":
                ...
                User admin = new User("admin","1234567890","admin", "admin@example.com", "admin", "example", "UNAL", true);
                User normal_user = new User("normal","9876543210", "user", "user@example.com", "user", "normal", "UNAL");
                items.add(admin);
